...
```bash
python3 plugins/semifluid/scripts/semifluid_api.py collections list --names
```
...
The helper sends bearer auth by default. `collections list` follows pagination automatically, and
`--names` emits only collection names for name-only requests. Run `auth login` once to authorize
through Semifluid OAuth; the helper stores credentials in a local cache and refreshes access tokens
when possible. For automation, the helper also accepts explicit bearer token overrides from
